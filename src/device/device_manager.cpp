...
OperatePLCWithVerify DeviceManager::operatePlcWithVerify(const std::string& deviceId, const std::string& cmd, const std::string& sensorId, const std::string& cameraId){
    ...
    OperateResult res = plcManager_->operate(deviceId,cmd);
    result.plcresult.deviceId = deviceId;
    result.plcresult.integrity = res.success;
    result.plcresult.message = res.message;
    PLCInfo status = plcManager_->getStatus(deviceId);
    auto itDeviceStatus = std::find_if(status.deviceStatuses.begin(), status.deviceStatuses.end(), 
                                           [&deviceId](const PLCDeviceStatus& ds) {
                                               return ds.id == deviceId;
    ...
    if(itDeviceStatus != status.deviceStatuses.end()){
        result.plcresult.status = itDeviceStatus->status;
    }
    else{
        result.plcresult.status = "UNKNOWN";
    }
    // result.plcresult = res;
    if(cmd == "ON"){
        auto start_time = std::chrono::steady_clock::now();
        auto timeout_time = start_time + std::chrono::seconds(10); 
        auto optionalInitialSensor = sensorManager_->getSensorDataRealTime(sensorId);
        if (!optionalInitialSensor.has_value()) {
            // 传感器数据获取失败
            result.plcresult.integrity = false;
            result.plcresult.message = "Failed to read sensor data for verification";
            result.isSuccess = false;
            return result;
        }
        SensorData initialSensor = optionalInitialSensor.value();
        ...
        while(std::chrono::steady_clock::now() < timeout_time){
            auto optCurrent = sensorManager_->getSensorDataRealTime(sensorId);
            if (!optCurrent.has_value()) {

                std::this_thread::sleep_for(std::chrono::milliseconds(100));
                continue;
            }

            currentSensor = *optCurrent;
